Helemaal goed — we gaan dit “gelijk trekken” naar één consistente set: **README + System Design + GitHub milestones/issues + labels**, gebaseerd op je bijgewerkte `todo.md` en guide. (Dit is al heel sterk en MVP-gericht.)

Hieronder krijg je copy-paste templates + een issue-structuur die 1:1 matcht met jouw todo/guide.

---

## 1) Repository structuur (docs)

Aanrader voor EHVCT repo:
...
* `README.md` = wat is het + hoe run je het
* `/docs/architecture.md` = system design (voor jou + later team)
* `/docs/domain.md` = data model + business rules
* `/docs/stripe.md` = integration + webhook flow
* `/docs/deployment.md` = checklist (Hostinger/VPS)
* `/docs/runbook.md` = “als X stuk is, check Y”

---

# 2) README.md (Recruiter + Client friendly)
